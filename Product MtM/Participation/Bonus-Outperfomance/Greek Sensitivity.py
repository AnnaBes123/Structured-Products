...
# --- Same product terms as "Bonus-Outperfomance Certificate.py" in this folder ---
STRIKE = 1.00
...
# T (TENOR) and STRIKE are fixed constants throughout this whole analysis -
# the only thing that varies below is spot. Vol and rate are also held at
# their inception values for every row.
...
def down_and_out_put_crr(S, K, H, T, r, sigma, steps, q=0.0):
    ...
    should be the number of remaining trading days. See
    "Bonus-Outperfomance Certificate.py" in this folder and its README for
    the full rationale (this replaced a continuous-monitoring closed form,
    which overstates the touch probability relative to what daily data can
    ever confirm). Assumes the barrier has NOT already been breached.
    """
    if S <= H:
        return 0.0
    if T <= 0:
...
def finite_difference_greeks_at(S, S0, T, sigma, breached, r=RISK_FREE_RATE, q=0.0, steps=None):
    ...
    There is no simple closed form once the barrier put is added, so
    unlike the plain Outperformance certificate, ALL FOUR Greeks here
    are numerical, not analytic: each is [price(x+bump) - price(x-bump)]
...
# GREEKS LADDER
#
# T (=TENOR) and K (=STRIKE*S0) are held constant throughout - the ONLY
# thing that varies across rows is spot. Vol and rate are also pinned at
# their inception values for every row. Each Greek at a given spot level
# IS the answer to "how much does MTM move for a 1-unit change in that
# variable, right now, at this spot": e.g. the Vega value in the row for
# spot = 80% of S0 tells you directly how many percentage points of MTM
# a 1% change in vol is worth if the index has already dropped 20%.
#
...
def plot_greek_sensitivity(table, S0, T, sigma, r, underlying_name):
    ...
    for ax, (col, ylabel) in zip(axes.flat, panels):
        ...
        ax.axvline(BARRIER * 100, color="dodgerblue", linewidth=0.8, linestyle="dotted")
    ...
    fig.suptitle(
        f"{underlying_name} Bonus-Outperformance Certificate - Greeks Ladder (T and strike fixed, spot varies)\n"
        f"Fixed throughout: T={T}y, vol={sigma:.2%}, r={r:.2%}, strike={STRIKE:.0%} of S0={S0:,.2f}, barrier={BARRIER:.0%}, "
        f"participation={OUTPERFORMANCE_PARTICIPATION:.0%}"
    )
    plt.tight_layout()
...
if __name__ == "__main__":
    print("Greeks Ladder - Bonus-Outperformance Certificate")
    print("=" * 60)
    ...
    print("then holds T, strike, vol and rate all fixed and varies ONLY spot,")
    print("so each Greek's value at a given spot level tells you directly how")
    print("much MTM moves for a 1-unit change in that variable at that spot")
    print("(e.g. the Vega row at spot=80% is what a 1% vol move is worth if")
    print("the index has already dropped 20%). Shown for both barrier states")
    print("where both are reachable (Not Breached / Breached).\n")
