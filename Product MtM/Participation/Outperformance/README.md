...
1. **Payoff if settled today** — the certificate's payoff formula applied to each day's index
   level, ignoring any time value (as if the certificate matured on that day).
2. **QuantLib fair value** — the certificate's actual mark-to-market value each day, priced as
   an option portfolio, including time value.
...
**Volatility** comes from the SPX implied-vol term structure — VIX (30-day), VIX3M (93-day),
and VIX6M (182-day) — interpolated each day to match that day's actual remaining time to
maturity (variance-time interpolation). Beyond 182 days, VIX6M is held flat. This avoids two
mistakes: using a single 30-day vol for a much longer holding period, and using the full
window's *realized* volatility, which would leak future information (e.g. a crash) into
valuations made before it happened.

At maturity, the fair value converges exactly to the settle-today payoff, since an option's
...
- **Dividend yield is fetched automatically** for the selected `TICKER` (0% for an index).
  ...
  meant the fair value at inception could look *above* par (100%) purely because the LEPO leg
...
- The certificate's fair value at inception (vs. par)
...
axis, and the certificate's fair value (as % of par) on its own right-hand axis. Every label
...
- **Right axis, solid line** — the certificate's actual fair value (% of par), converging onto
...
- **Rho (per 1% change in rates)**: percentage points of par moved per 1-percentage-point move
  ...
  rose from 4% to 5% right now, index unchanged, the certificate's fair value would rise by about
