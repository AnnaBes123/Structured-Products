...
# --- Product terms --- STRIKE (100% of S0, always) is the reference level
# BARRIER/BONUS_LEVEL are quoted against - the down-and-out put's actual
...
# REPLICATION: Long LEPO + Long Down-and-Out Put (struck at BONUS_LEVEL,
# barrier < BONUS_LEVEL). Uncapped: no short call leg. Payoff at maturity:
# never breached -> max(S_T, BONUS_LEVEL); breached -> S_T. Priced on a CRR
...
def lepo_price_and_greeks(S, T, q=0.0):
    """LEPO closed form, S*e^(-qT) - see ../../Yield/Discount
    Certificate/MATHEMATICS.md section 1."""
...
def down_and_out_put_crr(S, K, H, T, r, sigma, steps, q=0.0):
    ...
    this leg at 0 once the barrier has been touched. See ../../Yield/
    Barrier Reverse Convertible/MATHEMATICS.md."""
    if S <= H:
        return 0.0
    if T <= 0:
        return max(K - S, 0.0)
...
def verify_against_closed_form(S0, T, sigma, r=RISK_FREE_RATE, q=0.0, steps=None):
    ...
    plain vanilla put struck at BONUS_LEVEL."""
    K = BONUS_LEVEL * S0
...
def finite_difference_greeks_at(S, S0, T, sigma, breached, r=RISK_FREE_RATE, q=0.0, steps=None):
    """Central finite differences on the full certificate price - wider
    bumps than the closed-form products (CRR lattice sawtooth artifacts).
    See the Bullish Sharkfin README's bump-size scan."""
...
def plot_path(path, S0, underlying_name, mtm_vol_term_structure=None, greeks=None, q=0.0):
    ...
    if mtm_vol_term_structure is not None:
        ...
        mtm_line, = ax2.plot(
            ...
            linestyle="solid", label="Bonus Certificate — Model Value (% of Par)")
    ...
    ax.set_title(f"{underlying_name} Price Return Path from {path.index[0].date()} to {end_date.date()} \n"
                 f"vs Model Value vs. Par — Bonus Certificate (Uncapped)")
...
if __name__ == "__main__":
    ...
    summary = pd.Series({
        ...
        "Bonus Certificate — Redemption Payoff (vs. Par)": f"{certificate_return:.2%}",
    ...
    print(f"\nBonus Certificate — model value at inception")
