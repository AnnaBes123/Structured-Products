...
# ---------------------------------------------------------------------------
# REPLICATION: Bullish Sharkfin
#            = Long Zero-Coupon Bond (Principal)
#            + Long Up-and-Out Call (struck at STRIKE=100% of S0, barrier
#              BARRIER > STRIKE, checked once per trading day, optional
#              cash rebate paid immediately if the barrier is touched)
#
# The ZCB alone is what makes this a CAPITAL-PROTECTED note: it pays
# Principal at maturity regardless of anything the option leg does, so
# "if the underlying finishes below strike, capital is returned at 100%"
# falls straight out of the call being worthless there - no separate
# logic needed. This is the opposite financing story to every other
# barrier product in this repo (Barrier Reverse Convertible, Bonus
# Certificate, Bonus-Outperformance): there the option leg is SHORT and
# SUBTRACTED, financing an enhanced coupon at the cost of capital
# protection; here the option leg is LONG and ADDED on top of a fully
# protected bond, financed by accepting a cap on how far that
# participation can run (the barrier) instead of an outright cap level.
#
# As long as the index never trades up to the barrier, the investor gets
# FULL 1:1 upside participation above the strike - unlike the capped
# products in Product MtM/Participation/, there is no cap on the call
# itself, only the barrier's all-or-nothing cutoff. Touch the barrier once
# and the call is knocked out for good (the "fin" shape the product is
# named for: participation rises with the index, then drops to flat the
# instant the barrier is touched) - the note then just pays back principal
# at maturity, plus whatever rebate was already received at the moment of
# the knock-out (not part of the note's forward MTM from that point on,
# since it has already been paid).
#
# The call is priced on a Cox-Ross-Rubinstein binomial lattice (QuantLib
...
# mechanics; the same reasoning applies here unchanged, just mirrored onto
# an upper barrier instead of a lower one.
# ---------------------------------------------------------------------------

def up_and_out_call_crr(S, K, H, T, r, sigma, steps, rebate=REBATE, q=0.0):
    """
    Up-and-out call, strike K, barrier H > K, priced on a Cox-Ross-
...
def verify_against_closed_form(S0, T, sigma, r=RISK_FREE_RATE, q=0.0, steps=None):
    """
    Barrier pushed unreachable (H -> very high multiple of S0), rebate
    forced to 0: the up-and-out call can never knock out, so its CRR price
    should converge onto the plain vanilla call struck at STRIKE -
    confirms the barrier engine collapses to the ordinary closed form in
    the no-barrier limit, the same style of check used for the Barrier
    Reverse Convertible's down-and-in put and the Bonus Certificate's
    down-and-out put.
    """
    K = STRIKE * S0
...
def bullish_sharkfin_running_return(S0, path):
    ...
    spot, as a return over par (par/principal itself is always 100%, so
    this is directly "how much upside participation has this path locked
    in so far"). NOT what you'd actually receive if the note were sold or
    unwound today - it ignores all remaining time value in the still-live
    call, unlike the MTM fair-value line, which is the closest thing to an
    actual today's-value estimate. If the barrier has never been touched
    up to today, this is the call's intrinsic value, max(S - Strike, 0) -
    full upside participation. Once the barrier IS touched, the call is
    permanently dead - 0% here for every subsequent date, meaning exactly
    par (100% capital returned), regardless of where the index goes
    afterward.
    ...
    spikes through the barrier intraday and closes back below it does NOT
    knock the call out.
    """
...
def finite_difference_greeks_at(S, S0, T, sigma, breached, r=RISK_FREE_RATE, credit_spread=GS_CDS_SPREAD,
    ...
    Bump sizes here are much wider than the "0.1% of spot" convention used
    for the closed-form (non-barrier) products in this repo. A CRR barrier
    lattice is rebuilt from scratch on every call, and its node grid scales
    multiplicatively with both S and sigma (u=e^{sigma*sqrt(dt)}) - as
    those inputs vary continuously, which nodes fall above/below the FIXED
    strike/barrier changes in discrete jumps ("sawtooth" artifacts), not
    smoothly. A small bump can land entirely inside one of those jumps and
    return a Greek that's off by an order of magnitude. Empirically here
    (see the finite-difference bump-size scan in the PR/session that added
    this file): Delta was -0.17 at a 0.1%-of-S0 bump vs. a stable ~-0.02
    once the bump widens past roughly 1-2% of S0; Vega ranged from -517 to
    -2508 depending on bump size before settling near -700 around a
    2-vol-point bump; Rho was unaffected (r doesn't enter the lattice's
    node spacing, only drift/discounting) and keeps a small bump. These
    wider bumps trade some truncation error for a much larger reduction in
    lattice noise - the standard practitioner tradeoff for hedging barrier
    options priced on a binomial tree.
    """
...
def plot_path(path, S0, underlying_name, mtm_vol_term_structure=None, greeks=None, q=0.0):
    ...
    ax.plot(sharkfin_return_pct.index, sharkfin_return_pct.values, color="indianred", linewidth=1.5,
            linestyle="dashed", label="Bullish Sharkfin Participation Tracker")
    ...
    if mtm_vol_term_structure is not None:
        ...
        mtm_line, = ax2.plot(
            ...
            linestyle="solid", label="Bullish Sharkfin - Approximate MtM (% of Par)")
    ...
    ax.set_title(f"{underlying_name} Price Return Path from {path.index[0].date()} to {end_date.date()} \n"
                 f"vs Approximate Mark-to-Market (MtM) Value of Bullish Sharkfin")
...
if __name__ == "__main__":
    ...
    summary = pd.Series({
        ...
        "Bullish Sharkfin Return": f"{sharkfin_return:.2%}",
    ...
    print(f"\nBullish Sharkfin fair value at inception")
    print(f"(ZCB discounted at SOFR {RISK_FREE_RATE:.2%} + Goldman Sachs CDS {GS_CDS_SPREAD:.2%}, long")
    print(f"up-and-out call priced at SOFR alone via QuantLib's CRR binomial lattice (barrier checked")
