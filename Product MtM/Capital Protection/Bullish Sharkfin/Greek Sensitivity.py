...
# --- Same product terms as "Bullish Sharkfin.py" in this folder ---
STRIKE = 1.00
BARRIER = 1.20
REBATE = 0.0
RISK_FREE_RATE = 0.04
...
# also held at its inception value for every row. Spot never goes high
...
def up_and_out_call_crr(S, K, H, T, r, sigma, steps, rebate=REBATE, q=0.0):
    """
    Up-and-out call via a CRR binomial lattice (QuantLib
    BinomialCRRBarrierEngine), barrier checked once per step - steps
    should be the number of remaining trading days. See "Bullish
...
# an order of magnitude. See "Bullish Sharkfin.py" in this folder for the
# specific bump-size scan that motivated these wider bumps. Rho is
# unaffected (r doesn't enter the lattice's node spacing) and keeps a
# small bump.
# ---------------------------------------------------------------------------
