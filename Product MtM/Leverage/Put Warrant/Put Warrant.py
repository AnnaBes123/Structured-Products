...
# ---------------------------------------------------------------------------
# REPLICATION: Put Warrant = Long European Vanilla Put, struck at STRIKE
#
# The mirror image of the Call Warrant in the sibling folder - literally
# nothing more than a standard long vanilla put. No ZCB, no barrier, no
# participation multiple. This is a naked LONG OPTION POSITION, not a
# prepackaged debt instrument: there is no principal being lent to an
# issuer and no par value to speak of, unlike every other product in this
# repo. Accordingly this file does NOT express price/Greeks as a
# percentage of S0/par - there's no par here to express them against.
# Price and Greeks are reported in the same raw units a standard option
# pricing screen would use: Delta dimensionless, Vega/Rho per 1-
# percentage-point move, Theta per year/day, all in the underlying's own
# price units (index points, or $ for a single stock).
#
...
def put_warrant_intrinsic_value(S0, path):
    """
    The terminal payoff FORMULA applied to today's spot, max(Strike - S,
...
def plot_path(path, S0, underlying_name, mtm_vol_term_structure=None, greeks=None, q=0.0):
    ...
    if mtm_vol_term_structure is not None:
        ...
        mtm_line, = ax2.plot(
            ...
            linestyle="solid", label="Put Warrant - Approximate MtM")
    ...
    ax.set_title(f"{underlying_name} Price Return Path from {path.index[0].date()} to {end_date.date()} \n"
                 f"vs Approximate Mark-to-Market (MtM) Value of Put Warrant")
...
if __name__ == "__main__":
    ...
    print(f"\nPut Warrant fair value at inception")
