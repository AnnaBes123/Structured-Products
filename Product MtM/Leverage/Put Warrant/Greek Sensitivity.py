...
# --- Same product terms as "Put Warrant.py" in this folder ---
STRIKE = 1.00
RISK_FREE_RATE = 0.04
...
# T, strike and the funding curve are fixed constants throughout this
# whole analysis - the only thing that varies below is spot. Vol is also
# held at its inception value for every row.
...
# inception value for every row.
#
# Unlike every note/certificate product in this repo, there is no par
# value to express price/Greeks against - a warrant is a naked long
# option, not a prepackaged debt instrument. Price and Greeks are
# reported in RAW units here (underlying price points/$, standard
# per-1%-vol / per-1%-rate / per-year scalings), same convention as
# "Put Warrant.py".
# ---------------------------------------------------------------------------
...
def greek_sensitivity_table(S0, T, sigma, r=RISK_FREE_RATE, spot_multiples=SPOT_SCENARIO_RANGE, q=0.0):
    ...
    for multiple in spot_multiples:
        S = multiple * S0
        g = put_warrant_price(S, K, T, r, sigma, q)
        rows.append({
...
def plot_greek_sensitivity(table, S0, T, sigma, r, underlying_name):
    ...
    fig.suptitle(
        f"{underlying_name} Put Warrant - Greeks Ladder (T, strike fixed, spot varies)\n"
...
if __name__ == "__main__":
    print("Greeks Ladder - Put Warrant")
    print("=" * 60)
