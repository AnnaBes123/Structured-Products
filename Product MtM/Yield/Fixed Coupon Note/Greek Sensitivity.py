...
# --- Same product terms as "Fixed Coupon Note.py" in this folder ---
STRIKE = 0.90
...
# T (TENOR), strike, funding curve (1Y Treasury rate + CDS spread) are fixed constants
...
# GREEKS LADDER: T, strike, funding curve and vol pinned at entry; only spot
# varies; Theta excluded (T never moves). See README "Reading the charts".
...
def greek_sensitivity_table(S0, T, sigma, r=RISK_FREE_RATE, spot_multiples=SPOT_SCENARIO_RANGE, q=0.0):
    ...
    for multiple in spot_multiples:
        S = multiple * S0
        g = fixed_coupon_note_mtm(S, S0, T, sigma, r, q=q)
        rows.append({
...
def plot_greek_sensitivity(table, S0, T, sigma, r, underlying_name):
    ...
    fig.suptitle(
        f"{underlying_name} Fixed Coupon Note - Greeks Ladder (T and strike fixed, spot varies)\n"
...
if __name__ == "__main__":
    print("Greeks Ladder - Fixed Coupon Note")
    print("=" * 60)
