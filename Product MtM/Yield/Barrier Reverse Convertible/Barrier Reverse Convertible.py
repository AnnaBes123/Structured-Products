...
def plot_path(path, S0, underlying_name, mtm_vol_term_structure=None, greeks=None, q=0.0):
    ...
    ax.plot(brc_return_pct.index, brc_return_pct.values, color="indianred", linewidth=1.5,
            linestyle="dashed", label="Barrier Reverse Convertible Payoff If Settled Today (Relative to Par)")
...
if __name__ == "__main__":
    ...
    print(f"(ZCB discounted at the 1Y Treasury rate {RISK_FREE_RATE:.2%} + Goldman Sachs CDS {GS_CDS_SPREAD:.2%}, short")
    print(f"down-and-in put priced at the 1Y Treasury rate alone via QuantLib's CRR binomial lattice (barrier checked")
