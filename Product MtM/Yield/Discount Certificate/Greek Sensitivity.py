...
# --- Same product terms as "Discount Certificate.py" in this folder ---
DISCOUNT = 0.05                # target discount to spot at inception - CAP is derived from this
                                # via root-finding, same as the main backtest script; see
                                # solve_cap_for_discount and Discount Certificate.py for the
                                # full rationale (CAP, not DISCOUNT, is the actual strike here)
CAP = None                     # resolved from DISCOUNT at runtime - see __main__
RISK_FREE_RATE = 0.04
...
# T (TENOR) and CAP are fixed constants throughout this whole analysis -
...
def lepo_price_and_greeks(S, T, q=0.0):
    ...
    See Discount Certificate.py in this folder for the full rationale."""
...
def solve_cap_for_discount(S0, T, sigma, target_discount, r=RISK_FREE_RATE, q=0.0):
    """Solve for the CAP that makes fair value at inception equal
    S0*(1-target_discount) - see Discount Certificate.py for the full
    rationale. Fair value is monotonically increasing in CAP, so there's
    exactly one root in (100%, 1000% of S0]."""
...
# GREEKS LADDER
#
# T (=TENOR) and CAP are held constant throughout - the ONLY thing that
# varies across rows is spot. Vol and rate are also pinned at their
...
# right now, at this spot": e.g. the Vega value in the row for spot=120%
# tells you directly how many percentage points of MTM a 1% change in
# vol is worth once the index is 20% above where you bought the note.
#
...
def greek_sensitivity_table(S0, T, sigma, r=RISK_FREE_RATE, spot_multiples=SPOT_SCENARIO_RANGE, q=0.0):
    ...
    for multiple in spot_multiples:
        S = multiple * S0
        g = discount_certificate_mtm(S, S0, T, sigma, r, q=q)
        rows.append({
...
def plot_greek_sensitivity(table, S0, T, sigma, r, underlying_name):
    ...
    for ax, (col, ylabel, color) in zip(axes.flat, panels):
        ...
        ax.axvline(CAP * 100, color="dodgerblue", linewidth=0.8, linestyle="dotted")
    ...
    fig.suptitle(
        f"{underlying_name} Discount Certificate - Greeks Ladder (T and cap fixed, spot varies)\n"
        f"Fixed throughout: T={T}y, vol={sigma:.2%}, r={r:.2%}, cap={CAP:.0%} of S0={S0:,.2f}"
    )
    plt.tight_layout()
...
if __name__ == "__main__":
    print("Greeks Ladder - Discount Certificate")
    print("=" * 60)
    ...
    print("then holds T, cap, vol and rate all fixed and varies ONLY spot,")
    ...
    print(f"Resolved CAP for a {DISCOUNT:.2%} target discount: {CAP:.2%} of S0")

    print(f"\nFixed throughout (only spot varies below):")
    print(f"  Underlying:      {underlying_name} ({TICKER})")
    print(f"  Entry Date:      {ENTRY_DATE}")
    print(f"  S0 (spot):       {S0:,.2f}")
    print(f"  Cap (resolved):  {CAP:.0%} of S0 (target discount {DISCOUNT:.2%})")
    ...
    print(f"\nGreeks Ladder (QuantLib closed-form Black-Scholes-Merton - exact, not finite-difference,")
    print(f"since the discount certificate is a sum of plain calls with known analytic")
    print(f"derivatives):")
